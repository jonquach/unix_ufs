...
int main(int argc, char **argv) {
	...
	if (strcmp(argv[1],"blockfree")==0) {
		...
		printf("inode:%d size:%d blocks:%d nlink:%d\n",MyStat.st_ino, MyStat.st_size, MyStat.st_blocks, MyStat.st_nlink);
	} // ========== commande bd_ls() =============
...
/*				char LinkString[280];
				if (FileStat.st_mode&G_IFLNK)
					sprintf(LinkString,"%s"," -> ");
					bd_readlink(
				else {
					LinkString[0] = '\0';
				}				*/
				printf(" %c%c%c%c%c%c%c %14s size:%8d inode:%3d nlink:%2d \n",
				          (FileStat.st_mode&G_IFDIR )? 'd' : (FileStat.st_mode&G_IFLNK )? 'l' : '-',
	...
	}  /* ========== commande bd_chmod() pas supporté =============
		...
		if (argc!=4) {
			printf("La commande symlink demande 2 arguments!\n");
			printf("   ufs symlink fichier_existant lien_symbolique\n");
			return 1;
		}
		RetVal = bd_symlink(argv[2],argv[3]);
	} // ========== commande bd_readlink() =============
	else if (strcmp(argv[1],"readlink")==0) {
		if (argc!=3) {
			printf("La commande symlink demande 1 argument!\n");
			printf("   ufs readlink lien_symbolique\n");
			return 1;
		}
		char StringLink[256];
		RetVal = bd_readlink(argv[2],StringLink,256);
		if (RetVal>-1) {
			StringLink[RetVal] = '\0';
			printf("Le lien symbolique pointe vers %s\n",StringLink);
		}
