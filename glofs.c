...
static int glofs_getattr(const char *path, struct stat *stbuf)
{
	...
	if (RetVal < 0) {
		...
		if (MyStat.st_mode &	G_IFLNK) stbuf->st_mode = stbuf->st_mode | S_IFLNK;
...
static int glofs_symlink(const char *to, const char *linkName) {
	printf("symlink(%s,%s) appelé.\n",to,linkName);
	int RetCode = bd_symlink(to, linkName);
	return RetCode;
}

static int glofs_readlink(const char *path, char *buffer, size_t size) {
	printf("readlink(%s,%p,%d) appelé.\n",path, buffer, size);
	int RetCode = bd_readlink(path, buffer, size);
...
//	.chmod      = glofs_chmod,
	...
	.read       = glofs_read,
	.write      = glofs_write,
	.symlink    = glofs_symlink,
	.readlink   = glofs_readlink
};
