...
int getFreeInode();
void writeNewInode(iNodeEntry *iNodeEntry);
...
int getInodeEntry(ino iNodeNum, iNodeEntry *inodeEntry)
{
  ...
  if (getInodeBlockNumAndPos(iNodeNum, &iNodeBlockNum, &iNodePosition) == -1) return -1;
...
ino getInodeNumberFromPath(ino inode, char *pathToFind)
{
  if (strcmp(pathToFind, "/") == 0) return ROOT_INODE;
...
void writeNewInode(iNodeEntry *iNodeEntry) {
  ...
  printf("WriteNewInode\n");

  getInodeBlockNumAndPos(iNodeEntry->iNodeStat.st_ino, &iNodeBlockNum, &iNodePosition);

  ReadBlock(iNodeEntry->iNodeStat.st_ino, blockData);

  // iNodeEntry *iNodesBlock;

  // iNodesBlock = (struct iNodeEntry *) blockData;
  // *iNodeEntry = iNodesBlock[iNodePosition];

  printf("BLOCK DATA %s\n", blockData);

  // WriteBlock(iNodeEntry->iNodeStat.st_ino, blockData);
}
